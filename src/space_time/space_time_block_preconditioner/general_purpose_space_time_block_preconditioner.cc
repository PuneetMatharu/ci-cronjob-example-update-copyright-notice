...
namespace oomph
{
  ...
  void ExactDGPBlockPreconditioner<MATRIX>::setup()
  {
    ...
    t_subsidiary_setup_total+=(TimingHelpers::timer()-t_subsidiary_setup_start);
  ...
  //=============================================================================
  template<typename MATRIX>
  void ExactDGPBlockPreconditioner<MATRIX>::
  preconditioner_solve(const DoubleVector& r, DoubleVector& z)
  {
    ...
    this->Subsidiary_preconditioner_pt[0]->
    preconditioner_solve(rhs_reordered,z_reordered);
  ...
  void BandedBlockTriangularPreconditioner<MATRIX>::setup()
  {
    ...
    for (unsigned i=0; i<n_block_types; i++)
    {
          ...
          (this->Subsidiary_preconditioner_pt[i])!=0)
      {
        ...
        if (Compute_memory_statistics)
        {
          ...
          {
            // Enable the doc-ing of memory usage in the GMRES block preconditioner
            dynamic_cast<GMRESBlockPreconditioner*>
            (this->Subsidiary_preconditioner_pt[i])->enable_doc_memory_usage();
          }
        } // if (Compute_memory_statistics)
    ...
    if (Compute_memory_statistics)
    {
      ...
      // NOTE: This calculation is done by taking into account the storage scheme
      // for a compressed row matrix
      ...
      for (unsigned i=0; i<n_block_types; i++)
      {
        // See if the i-th subsidiary preconditioner the GMRES block preconditioner
        if (dynamic_cast<GMRESBlockPreconditioner*>
            (this->Subsidiary_preconditioner_pt[i])!=0)
        {
        ...
        else
        {
          ...
          if (!have_issued_warning_about_memory_calculations)
          {
            ...
            warning_message_stream << "Can't compute the memory statistics for the "
                                   << i << "-th diagonal block in the banded\nblock "
  ...
  //=============================================================================
  template<typename MATRIX>
  void BandedBlockTriangularPreconditioner<MATRIX>::
  preconditioner_solve(const DoubleVector& r, DoubleVector& z)
  {
    ...
    for (int i=start; i!=end; i+=step)
    {
          ...
          (this->Subsidiary_preconditioner_pt[i])!=0)
      {
        ...
        iteration_count[i]=dynamic_cast<GMRESBlockPreconditioner*>
                           (this->Subsidiary_preconditioner_pt[i])->iterations();
    ...
    if (n_block_solved_with_gmres>0)
    {
      ...
      for (unsigned i=0; i<n_block; i++)
      {
            ...
            (this->Subsidiary_preconditioner_pt[i])!=0)
        {
          ...
          if (iteration_count[i]<0)
          {
            ...
            error_message_stream << "Iteration count should be non-negative but "
                                 << "you have an iteration\ncount of "
                                 << iteration_count[i] << "!" << std::endl;
      ...
      for (unsigned i=0; i<n_block; i++)
      {
        ...
        {
          // Update the mean
          n_iter_mean+=(double(iteration_count[i])/double(n_block_solved_with_gmres));
        } // if (dynamic_cast<GMRESBlockPreconditioner*>...
      } // for (unsigned i=0;i<n_block;i++)
      ...
      for (unsigned i=0; i<n_block; i++)
      {
        ...
        {
          // Update the variance
          n_iter_var+=(std::pow(double(iteration_count[i])-n_iter_mean,2.0)/
