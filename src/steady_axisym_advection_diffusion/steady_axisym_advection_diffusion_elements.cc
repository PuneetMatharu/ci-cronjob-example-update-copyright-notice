...
//LIC// 
//LIC//====================================================================
//Non-inline functions for Axisymmetric Advection Diffusion elements
...
double SteadyAxisymAdvectionDiffusionEquations::
 ...
 for(unsigned ipt=0;ipt<n_intpt;ipt++)
  {
   ...
   get_wind_axisym_adv_diff(ipt,s,interpolated_x,wind);
     ...
     if(local_eqn >= 0)
      {
       ...
       residuals[local_eqn] += r*source*test(l)*W;
       
       //The Advection Diffusion bit itself
       for(unsigned k=0;k<2;k++)
        {
         //Construct the contribution to the residuals
         residuals[local_eqn] -= 
          r*interpolated_dudx[k]*(scaled_peclet*wind[k]*test(l) + dtestdx(l,k))*W;
        }
             ...
             for(unsigned i=0;i<2;i++)
              {
               //Assemble Jacobian term
                jacobian(local_eqn,local_unknown) -= 
                 r*dpsidx(l2,i)*(scaled_peclet*wind[i]*test(l) + dtestdx(l,i))*W;              
              }
            }
          }
        }
...
unsigned  SteadyAxisymAdvectionDiffusionEquations::self_test()
{
 ...
 for (unsigned iplot=0;iplot<num_plot_points;iplot++)
  {
   ...
   unsigned ipt = 0;
   get_wind_axisym_adv_diff(ipt,s,x,wind);
   for(unsigned i=0;i<2;i++) 
    {
     outfile << wind[i] << " ";
    }
...
const unsigned QSteadyAxisymAdvectionDiffusionElement<NNODE_1D>::
Initial_Nvalue = 1;
...
template class QSteadyAxisymAdvectionDiffusionElement<2>;
template class QSteadyAxisymAdvectionDiffusionElement<3>;
template class QSteadyAxisymAdvectionDiffusionElement<4>;
