...
for(unsigned ipt=0;ipt<n_intpt;ipt++)
{
 ...
 if(!this->Use_extrapolated_strainrate_to_compute_second_invariant)
  ...
  {   
   this->extrapolated_strain_rate(ipt,strainrate_to_compute_second_invariant);
  }
         ...
         if(this->Pre_multiply_by_viscosity_ratio)
          {
           sum = visc_ratio*viscosity*
            (sum*testf[l] + interpolated_p*dtestfdx(l,i))*W*hang_weight;
          }
         else
          {
                 ...
                 if(local_unknown >= 0)
                  {
                   ...
                   jacobian(local_eqn,local_unknown)
                    -= visc_ratio*viscosity*this->Gamma[i]*dpsifdx(l2,i)*
...
//======================================================================
template<>
void PRefineableGeneralisedNewtonianQCrouzeixRaviartElement<2>::further_build()
{
 ...
 if(this->tree_pt()->father_pt()!=0)
  {
   ...
   PRefineableGeneralisedNewtonianQCrouzeixRaviartElement<2>* father_element_pt=
    dynamic_cast<PRefineableGeneralisedNewtonianQCrouzeixRaviartElement<2>*>
...
//======================================================================
template<>
void PRefineableGeneralisedNewtonianQCrouzeixRaviartElement<3>::further_build()
{
 if (this->tree_pt()->father_pt()!=0)
  {
   //Call the generic further build (if there is a father)
 ...
 if(this->tree_pt()->father_pt()!=0)
  {
   // Pointer to my father (in C-R element impersonation)
   PRefineableGeneralisedNewtonianQCrouzeixRaviartElement<3>* father_element_pt=
    dynamic_cast<PRefineableGeneralisedNewtonianQCrouzeixRaviartElement<3>*>
