 ...
 for(unsigned l=0;l<n_node;l++) 
  {
   ...
   dudt += this->du_dt_adv_diff(l)*psi(l);
     ...
     if(local_eqn >= 0) 
      {
       ...
       //The Mesh velocity and Advection--Diffusion bit
       for(unsigned k=0;k<DIM;k++)
        {
         ...
         residuals[local_eqn] -= interpolated_dudx[k]*
          (tmp*test(l) + dtestdx(l,k))*W*hang_weight;
        }
                 ...
                 for(unsigned k=0;k<DIM;k++)
                  {
                   //Terms that multiply the test function
                   double tmp = peclet*wind[k];
                   //If the mesh is moving, subtract the mesh velocity
                   if(!ALE_is_disabled_flag) 
                    {tmp -= peclet_st*mesh_velocity[k];}
                   //Construct the jacobian term
                   jacobian(local_eqn,local_unknown) -=
                    dpsidx(l2,k)*(tmp*test(l) + dtestdx(l,k))*W*
...
//====================================================================
template class RefineableQAdvectionDiffusionElement<2,2>;
template class RefineableQAdvectionDiffusionElement<2,3>;
template class RefineableQAdvectionDiffusionElement<2,4>;

template class RefineableQAdvectionDiffusionElement<3,2>;
template class RefineableQAdvectionDiffusionElement<3,3>;
template class RefineableQAdvectionDiffusionElement<3,4>;
